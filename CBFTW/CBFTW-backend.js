...
// This method checks if the given record exists and executes callback 
// depending on success / failure
async function checkIfAlreadyExists(ID, success, failure) {
...
async function invoke(request, socket) {
    ...
    if (isProposalGood) {
        ...
        var transaction_id_string = tx_id.getTransactionID(); //Get the transaction ID string to be used by the 
        // event processing
        var promises = [];
        ...
        let txPromise = new Promise((resolve, reject) => {
            let handle = setTimeout(() => {
                ...
                resolve({ event_status: 'TIMEOUT' }); //we could use reject(new Error('Trnasaction 
...
// This method takes in the the socket (to respond to client) and the name of the user to be enrolled. 
// It returns the user if successful
...
async function getUser(socket, user) {
    ...
    if (user_from_store && user_from_store.isEnrolled()) {
        ...
        const eh = channel.newChannelEventHub(peer)
	console.log('The eh value', eh);
        let abcd = eh.connect();
	console.log('The connection', abcd, peer);
    ...
    socket.on('REQUEST', (req) => {
        switch (req.action) {
            case "QUERY":
                ...
                checkIfAlreadyExists(req.data.ID,
                    () => {
                        query({
                            chaincodeId: 'banks',
                            fcn: 'query',
            ...
            case "CREATECUSTOMER":
                        ...
                        invoke(
                            {
                                chaincodeId: 'banks',
                                fcn: 'createCustomer',
                                args: [req.data.cname, req.data.ID, req.data.ccountry, req.data.ccurrency, req.data.balance, req.data.customerBankID],
                                chainId: 'mychannel',
                            }
                            , socket)
                    });
                break;
            case "CREATEBANK":
                socket.emit('RESPONSE', { type: 'START', payload: `Request for CREATE for ${req.data.bankID} received` });
                checkIfAlreadyExists(req.data.bankID,
                    () => {
                        socket.emit('RESPONSE', { type: 'ERROR', payload: `${req.data.bankID} ALREADY EXISTS!` });
                    },
                    () => {
                        invoke(
                            {
                                chaincodeId: 'banks',
                                fcn: 'createBank',
                                args: [req.data.bankID, req.data.bname, req.data.bcountry, req.data.bcurrency, req.data.reserves],
                                chainId: 'mychannel',
                            }
                            , socket)
                    });
                break;
            case "CREATEFOREX":
                socket.emit('RESPONSE', { type: 'START', payload: `Request for CREATE for ${req.data.ename} received` });
                checkIfAlreadyExists(req.data.ename,
                    () => {
                        socket.emit('RESPONSE', { type: 'ERROR', payload: `${req.data.ename} ALREADY EXISTS!` });
                    },
                    () => {
                        invoke(
                            {
                                chaincodeId: 'banks',
                                fcn: 'createForex',
                                args: [req.data.ename, req.data.rate],
                                chainId: 'mychannel',
                            }
                            , socket)
                    });
                break;
            case "PAY":
                console.log('lets see what will be printed',req.data, req.data.fromUserId, req.data.toUserId, req.data.amount);
                socket.emit('RESPONSE', { type: 'START', payload: `Request for CREATE for ${req.data.bname} received` });
                        invoke(
                            {
                                chaincodeId: 'banks',
                                fcn: 'pay',
                                args: [req.data.fromUserID, req.data.toUserID, req.data.amount],
                                chainId: 'mychannel',
                            }, socket)
                break;
