...
func (s *SmartContract) initLedger(APIstub shim.ChaincodeStubInterface) sc.Response {
        banks := []Bank{
                {BName: "US_Bank", BankID: "US_Bank", Country: "USA", Currency: "USD", Reserves: 1000000.0},
                {BName: "UK_Bank", BankID: "UK_Bank", Country: "UK ", Currency: "GBP", Reserves: 1000000.0},
                {BName: "Japan_Bank", BankID: "Japan_Bank", Country: "JAPAN", Currency: "JPY", Reserves: 1000000.0},
        }

        customers := []Customer{
                {CName: "US_John_Doe", CustID: "123", Country: "US", Currency: "USD", Balance: 10000.0, CustomerBankID: "US_Bank"},
                {CName: "US_Alice", CustID: "456", Country: "US", Currency: "USD", Balance: 10000.0, CustomerBankID: "US_Bank"},
                {CName: "UK_John_Doe", CustID: "123", Country: "UK", Currency: "GBP", Balance: 10000.0, CustomerBankID: "UK_Bank"},
                {CName: "UK_Alice", CustID: "456", Country: "UK", Currency: "GBP", Balance: 10000.0, CustomerBankID: "UK_Bank"},
                {CName: "JPY_John_Doe", CustID: "123", Country: "Japan", Currency: "JPY", Balance: 10000.0, CustomerBankID: "Japan_Bank"},
                {CName: "JPY_Alice", CustID: "456", Country: "Japan", Currency: "JPY", Balance: 10000.0, CustomerBankID: "Japan_Bank"},
        }
        ...
        forex := []Forex{
                ...
                /* the following are needed in case 
                payments are in the same currency */
...
func writeCustomerToLedger(APIStub shim.ChaincodeStubInterface, customers []Customer) sc.Response {
        for i := 0; i < len(customers); i++ {
                key := customers[i].CName + "_" + customers[i].CustID
                chkBytes, _ := APIStub.GetState(key)
                if chkBytes == nil { //only add if it is not already present
                        asBytes, _ := json.Marshal(customers[i])
                        err := APIStub.PutState(customers[i].CName+"_"+customers[i].CustID, asBytes)
                        if err != nil {
                                return shim.Error(err.Error())
                        }
                } else {
...
func (s *SmartContract) pay(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
        ...
        fromCustomerName := fromCustomer.CName
        fromCustomerCustID := fromCustomer.CustID
        fromCurrency := fromCustomer.CCurrency
        ...
        if fromBalance < payAmount {
                errMsg := "Insufficent funds in customer: " + fromCustomerCName + " Customer ID: " + fromCustomerCustID
        ...
        toCustomerName := toCustomer.CName
        toCurrency := toCustomer.CCurrency
        ...
        if fromBankReserves < payAmount {
                errMsg := "Insufficent funds in bank reserves: " + fromCustomerBank.BName + " Bank ID: " + fromCustomerBank.BankID
                return shim.Error(errMsg)
        }
        ...
        if err != nil {
                return shim.Error("Error writing updates to FROM customer account " + fromCustomer.CName)
        }

        toCustAsBytes, _ = json.Marshal(toCustomer)
        err = APIstub.PutState(args[1], toCustAsBytes)
        if err != nil {
                return shim.Error("Error writing updates to TO customer account " + toCustomer.CName)
        }
        fromCustBankAsBytes, _ = json.Marshal(fromCustomerBank)
        err = APIstub.PutState(fromBank, fromCustBankAsBytes)
        if err != nil {
                return shim.Error("Error writing updates to FROM Bank account " + fromCustomerBank.BName)
        }
        toCustBankAsBytes, _ = json.Marshal(toCustomerBank)
        err = APIstub.PutState(toBank, toCustBankAsBytes)
        if err != nil {
                return shim.Error("Error writing updates to TO Bank account " + toCustomerBank.BName)
        }

        if err == nil {
                fmt.Println("~~~~~~~~~~~~~~~~~ Success fully transferred: ", payAmount, " From: ", fromCustomerCName, " TO: ", toCustomerCName, "~~~~~~~~~~~~~~~~~")
        }
...
func (s *SmartContract) createBank(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
        ...
        banks := []Bank{Bank{BName: args[0], BankID: args[1], Country: args[2], Currency: args[3], Reserves: reserves}}
...
func (s *SmartContract) createCustomer(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
        ...
        customers := []Customer{{CName: args[0], CustID: args[1], Country: args[2], Currency: args[3], Balance: balance, CustomerBankID: args[5]}}
